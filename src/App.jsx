...
const VOICES = [
  { id: 'Puck', gender: 'Pria', label: 'Energik & Ramah' },
  { id: 'Charon', gender: 'Pria', label: 'Tegas & Berwibawa' },
  { id: 'Orus', gender: 'Pria', label: 'Hangat & Santai' },
  { id: 'Zephyr', gender: 'Wanita', label: 'Lembut & Tenang' },
  { id: 'Kore', gender: 'Wanita', label: 'Profesional & Jelas' },
  { id: 'Aoede', gender: 'Wanita', label: 'Ceria & Ekspresif' }
];
...
// Mengonversi base64 PCM (raw audio dari Gemini TTS) menjadi file WAV yang bisa diputar browser
const pcmBase64ToWavBlob = (base64Data, sampleRate = 24000, numChannels = 1, bitsPerSample = 16) => {
...
export default function App() {
  ...
  const handleGenerateAudio = async () => {
    if (!jsonResult?.voice_over_script) return;
    if (!apiKey) { setShowApiModal(true); return; }
    setTtsError(''); setTtsLoading(true); setAudioUrl('');
    try {
      const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key=${apiKey}`;
      const payload = {
        contents: [{ parts: [{ text: jsonResult.voice_over_script }] }],
        generationConfig: {
      ...
      const audioPart = data.candidates?.[0]?.content?.parts?.[0]?.inlineData?.data;
      if (!audioPart) throw new Error('Audio tidak ditemukan di response.');
      const blob = pcmBase64ToWavBlob(audioPart);
      setAudioUrl(URL.createObjectURL(blob));
    } catch (err) {
      console.error(err);
      setTtsError('Gagal membuat suara. Cek API key atau kuota TTS harian.');
    } finally {
      setTtsLoading(false);
    }
  };
          ...
          {jsonResult && (
            <div className="grid lg:grid-cols-5 gap-6 mt-12 pb-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="lg:col-span-3 bg-[#141414] rounded-2xl p-6 md:p-8 border border-[#2a2a2a] shadow-2xl space-y-8">
                ...
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-emerald-500 mb-3 flex items-center"><Sparkles className="w-4 h-4 mr-2"/> Text-to-Speech (Gratis, Gemini TTS)</h3>
                  <div className="p-5 bg-[#0a0a0a] rounded-xl border border-[#333] space-y-3">
                    <div className="grid grid-cols-3 gap-2">
                      {VOICES.map(v => (
                        <div key={v.id} onClick={() => setSelectedVoice(v.id)}
                          className={`p-2.5 rounded-lg cursor-pointer border text-center transition-all ${selectedVoice === v.id ? 'border-emerald-500 bg-emerald-500/10' : 'border-[#2a2a2a] hover:border-[#444]'}`}>
                          <p className={`text-xs font-bold ${selectedVoice === v.id ? 'text-emerald-400' : 'text-gray-300'}`}>{v.id}</p>
                          <p className="text-[10px] text-gray-500">{v.gender} · {v.label}</p>
                        </div>
                      ))}
                    </div>
                    <button onClick={handleGenerateAudio} disabled={ttsLoading}
                      className={`w-full py-2.5 rounded-lg font-bold text-sm flex items-center justify-center transition-all ${ttsLoading ? 'bg-gray-700 text-gray-400' : 'bg-emerald-500 text-black hover:bg-emerald-400'}`}>
                      {ttsLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Membuat suara...</> : <><Music className="w-4 h-4 mr-2" /> Buat Suara (VO)</>}
                    </button>
                    {ttsError && <p className="text-red-400 text-xs">{ttsError}</p>}
                    {audioUrl && (
                      <div className="space-y-2">
                        <audio controls src={audioUrl} className="w-full" />
                        <a href={audioUrl} download="voice_over.wav" className="block text-center text-xs font-semibold text-emerald-400 hover:text-emerald-300">⬇ Download voice_over.wav</a>
                      </div>
                    )}
                    <p className="text-[10px] text-gray-600">Suara otomatis mengikuti aksen bahasa naskah (Indonesia). Kuota gratis: ~15 request/menit via Gemini API.</p>
                  </div>
                </div>
              </div>
